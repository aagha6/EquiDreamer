...
class GaussianSmoothing:
    ...
    def _add_current_episode(self, worker):
        ...
        rew_smooth = gaussian_filter1d(
            np.array(self.rew_buffer[worker]), self.sigma, mode="nearest"
        )
...
class ExponentialSmoothing:
    ...
    def add(self, step, worker=0, load=False):
        ...
        rew_smooth = (
            self.alpha * self.rew_buffer.get(worker, current_rew)
            + self.beta * current_rew
        )
...
class UniformSmoothing:
    def __init__(self, replay, delta=0, kernel_type="uniform"):
        ...
        assert (
            self.delta > 0
        ), "make sure window for uniform smoothing is an integer > 0"
        if kernel_type == "uniform":
            self.filter = (1.0 / self.delta) * np.array([1] * self.delta)
        elif kernel_type == "uniform_before":
            self.filter = (1.0 / self.delta) * np.array(
                [1] * self.delta + [0] * (self.delta - 1)
            )
        elif kernel_type == "uniform_after":
            self.filter = (1.0 / self.delta) * np.array(
                [0] * (self.delta - 1) + [1] * self.delta
            )
        else:
            raise NotImplementedError(
                "invalid kernel_type setting for uniform smoothing"
            )
    ...
    def _add_current_episode(self, worker):
        ...
        rew_smooth = convolve(
            np.array(self.rew_buffer[worker]), self.filter, mode="nearest"
        )
        for i, step in enumerate(self.current_transitions[worker]):
            kwargs = step.pop("kwargs")
            rew_raw = step["reward"]
