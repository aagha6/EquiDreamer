...
class RSSM(nj.Module):
  ...
  def _conv_gru(self, x, deter):
    ...
    kw = {'in_channels': self._deter + self._kw['units'], 
  ...
  def proto_loss(self, post, obs_proj, ema_proj):
    ...
    if self._equiv:
      ...
      obs_scores = jax.vmap(self._prototypes, [1])(obs_proj).transpose(2,0,1)
      obs_scores = obs_scores.reshape([-1, obs_scores.shape[-1]])
      obs_scores = jnp.reshape(obs_scores, [self._num_prototypes * self._grp.scaler, B, T])
    else:
    ...
    if self._equiv:
      ...
      ema_scores = jax.vmap(self._prototypes, [1])(ema_proj).transpose(2,0,1)
      ema_scores = ema_scores.reshape([-1, ema_scores.shape[-1]])
      ema_scores = jnp.reshape(ema_scores, [self._num_prototypes * self._grp.scaler, B, T])
    else:
    ...
    if self._equiv:
      ...
      feat_scores = jax.vmap(self._prototypes, [1])(feat_proj).transpose(2,0,1)
      feat_scores = feat_scores.reshape([-1, feat_scores.shape[-1]])
      feat_scores = jnp.reshape(feat_scores, [self._num_prototypes * self._grp.scaler, B, T])
    else:
...
class EquivMLP(MLP):
  ...
  def __init__(
      ...
      if invariant:
        ...
        self._init_equiv_std = nn.R2Conv(in_type=self.feat_type_hidden,
...
class Dist(nj.Module):
  ...
  def __init__(
    ...
    if dist=='equiv_normal':
      assert in_type is not None and out_type is not None and init_equiv_actor is not None
    ...
    if self._dist == 'equiv_normal':
      ...
      out = self.get('out', 
                  ...
                  **{"net":self._init_equiv_actor, 
                  ...
                  'norm':'none',
                  'act':'none'})(inputs)
      out = out.reshape(inputs_shape[:-1] + (out.shape[-1],)).astype(f32)
    else:
    ...
    if self._dist == 'equiv_normal':
      std = self.get('std', 
                  ...
                  **{"net":self._init_equiv_std, 
                  ...
                  'norm':'none',
                  'act':'none'})(inputs)
      std = std.reshape(inputs_shape[:-1] + (std.shape[-1],)).astype(f32)
